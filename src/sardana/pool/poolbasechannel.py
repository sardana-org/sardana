...
class Value(BufferedAttribute):
    ...
    def create_value_buffer(self):
        ...
        return ValueBuffer(listeners=self.on_change, persistent=persistent)
...
class PoolBaseChannel(PoolElement):
    ...
    def extend_value_buffer(self, values, idx=None, propagate=1):
        ...
        if len(values) == 0:
            return
        val_attr = self._value
        val_attr.extend_value_buffer(values, idx, propagate)
    ...
    def append_value_buffer(self, value, idx=None, propagate=1):
        ...
        """
        val_attr = self._value
        val_attr.append_value_buffer(value, idx, propagate)
        val_attr.set_value(value, propagate=propagate)

    def reset_value_buffer(self):
        val_attr = self._value
        val_attr.reset_value_buffer()
