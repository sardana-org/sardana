...
class DiffracBasis(PseudoMotorController):
  ...
  class_prop = {'DiffractometerType' : {Type:str, Description : 'Type of the diffractometer, e.g. E6C'},}
...
#                                         Memorize: MemorizedNoInit, # Comment it out if you don't want to load the last loaded reflection file automatically 
                                          Access :ReadWrite},
  ...
  def __init__(self,inst,props, *args, **kwargs):
    ...
    self.samples_list["default_crystal"] =  Hkl.Sample.new("default_crystal")
#    self.sample = Hkl.Sample.new("default_crystal")
    ...
    # Engine hkl -> it has not effect, it will be set the last value set (stored in DB)
    ...
    try:
      if self.nb_ph_axes == 4: # "E4CV", "E4CH", "SOLEIL MARS": hkl(3), psi(1), q(1); "K4CV": hkl(3), psi(1), q(1), eulerians(3); "SOLEIL SIXS MED1+2": hkl(3), q2(2), qper_qpar(2) 
        if self.engine.name() == "hkl":
          self.engine.set_values_unit([pseudos[0],pseudos[1],pseudos[2]]) #compute the HklGeometry angles for this HklEngine
          ...
          self.engine.set_values_unit([pseudos[5],pseudos[6],pseudos[7]])
        ...
        if self.engine.name() == "hkl":
          self.engine.set_values_unit([pseudos[0],pseudos[1],pseudos[2]])
        elif self.engine.name() == "psi":
          self.engine.set_values_unit([pseudos[3]])
        elif self.engine.name() == "q2":
          ...
          self.engine.set_values_unit([pseudos[8],pseudos[9],pseudos[10]])
      else:
        if self.engine.name() == "hkl":
          self.engine.set_values_unit([pseudos[0],pseudos[1],pseudos[2]])
  ...
  def calc_all_pseudo(self, physicals):
    ...
    # Calcular hkl
    if self.nb_ph_axes == 6:
      self.geometry.set_axes_values_unit([mu,th,chi,phi,gamma,delta])
  ...
  def setComputeTrajectoriesSim(self, value):
    ...
    if self.engine.name() == "hkl":
      if len(value) < 3:
        raise Exception("Not enough parameters given. Three are necessary: h, k, l ")
      else:
        try:
          self.engine.set_values_unit([value[0],value[1],value[2]]) #compute the HklGeometry angles for this HklEngine
        except GLib.GError,err:
      ...
      if len(value) < 3:
        raise Exception("Not enough parameters given. Three are necessary for eulerians mode")
      else:
        try:
      ...
      if len(value) < 2:
        raise Exception("Not enough parameters given. Three are necessary for q2 mode")
      else:
        try:
      ...
      if len(value) < 2:
        raise Exception("Not enough parameters given. Three are necessary for qper_qpar mode")
      else:
        try:
          self.engine.set_values_unit([value[0],value[1]])
        except GLib.GError,err:
  ...
  def setAddReflection(self,value):
    ...
    newref= self.sample.add_reflection(self.geometry,self.detector,value[0], value[1], value[2])
    # Set affinement if given (by default reflections are created with affinement = 1)
    if len(value) > 3: 
  ...
  def setLoadReflections(self, value): # value: complete path of the file with the reflections to set
    ...
    for line in reflections_file:
      ...
      newref= self.sample.add_reflection(self.geometry,self.detector,ref_values[1], ref_values[2], ref_values[3])
      # Set affinement
  ...
  def getReflectionAngles(self):
    ...
    for ref1 in self.sample.reflections_get():
      ...
      for ref2 in self.sample.reflections_get():
        ...
        angle = 0
        if i < j:
          angle = self.sample.get_reflection_mesured_angle(ref1,ref2)
        elif j < i:
          angle = self.sample.get_reflection_theoretical_angle(ref1,ref2)
