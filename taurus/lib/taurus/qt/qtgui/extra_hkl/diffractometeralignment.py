...
# Code implementation generated from reading ui file 'diffractometeralignment.ui'
#
# Created: Tue Aug 13 09:55:29 2013 
...
from ui.ui_diffractometeralignment import Ui_DiffractometerAlignment
...
class DiffractometerAlignment(TaurusWidget):
    ...
    def __init__(self, parent=None, designMode=False):
        ...
        self.connect(self._ui.AlignmentStopButton, Qt.SIGNAL("clicked()"), self.stop_movements)  
        self.connect(self._ui.AlignmentStoreReflectionButton, Qt.SIGNAL("clicked()"), self.store_reflection)

        self.connect(self._ui.MacroServerConnectionButton, Qt.SIGNAL("clicked()"), self.open_macroserver_connection_panel)

        self.connect(self._ui.SelectSignalButton, Qt.SIGNAL("clicked()"), self.open_selectsignal_panel)
                    
        #Create a global SharedDataManager
        Qt.qApp.SDM =  SharedDataManager(self)

        self.door_device = None
        self.door_device_name = None  
    ...
    def setModel(self,model):
     ...
     # Add dynamically the angle widgets
        ...
        for motor in self.device.motorlist:
            ...
            self.motor_devices.append(taurus.Device(self.motor_names[len(self.motor_names) - 1]))
        ...
        for i in range(0,self.nb_motors):
            ...
            angles_labels[i].setGeometry(QtCore.QRect(150 + gap_x * i, 40, 71, 17))
            angles_labels[i].setLayoutDirection(QtCore.Qt.RightToLeft)
            alname = "angleslabel" + str(i)
            angles_labels[i].setObjectName(alname)
            angles_labels[i].setText(QtGui.QApplication.translate("HKLScan", self.angles_names[i], None, QtGui.QApplication.UnicodeUTF8))

            angles_taurus_label.append(TaurusLabel(self))
            angles_taurus_label[i].setGeometry(QtCore.QRect(150 + gap_x * i , 70, 81, 19))
            ...
            angles_taurus_input[i].setGeometry(QtCore.QRect(145 + gap_x * i , 100, 91, 27))
       ...
       # Set model to engine and modes
        ...
        exec_functions = [self.exec_scan1, self.exec_scan2, self.exec_scan3, self.exec_scan4, self.exec_scan5, self.exec_scan6]

        tomax_functions = [self.tomax_scan1, self.tomax_scan2, self.tomax_scan3, self.tomax_scan4, self.tomax_scan5, self.tomax_scan6]
        
        gap_x = 650/self.nb_motors
        
        for i in range(0,self.nb_motors):
            scan_buttons.append(QtGui.QPushButton(self))
            scan_buttons[i].setGeometry(QtCore.QRect(150 + gap_x * i, 405, 100, 26))
            wname = "scanbutton" + str(i)
            scan_buttons[i].setObjectName(wname)
            scan_buttons[i].setText(QtGui.QApplication.translate("DiffractometerAlignment", self.angles_names[i], None, QtGui.QApplication.UnicodeUTF8))
            self.connect(scan_buttons[i], Qt.SIGNAL("clicked()"), exec_functions[i]) 

            self.range_inputs.append(QtGui.QLineEdit(self))
            self.range_inputs[i].setGeometry(QtCore.QRect(150 + gap_x * i, 440, 100, 26))
            ...
            self.tomax_buttons[i].setGeometry(QtCore.QRect(150 + gap_x * i, 475, 100, 26))
            wname = "tomaxbutton" + str(i)
            self.tomax_buttons[i].setObjectName(wname)
            self.tomax_buttons[i].setText(QtGui.QApplication.translate("DiffractometerAlignment", 'n.n.', None, QtGui.QApplication.UnicodeUTF8))
            self.connect(self.tomax_buttons[i], Qt.SIGNAL("clicked()"), tomax_functions[i]) 
    ...
    def exec_scan(self, imot):
        ...
        macro_command.append(str(self.selectsignal._ui.SampleTimelineEdit.text()))
        ...
        output_values = self.door_device.getAttribute("Output").read().value
        if output_values != None:
            for i in range(len(output_values)):
                if output_values[i] == "Position to move":
                    self.tomax_buttons[imot].setText(QtGui.QApplication.translate("DiffractometerAlignment", str(output_values[i+1]), None, QtGui.QApplication.UnicodeUTF8))
    ...
    def open_macroserver_connection_panel(self):
        ...
        Qt.qApp.SDM.connectWriter("macroserverName", w, 'macroserverNameChanged')
