...
# Code implementation generated from reading ui file 'hklscan.ui'
#
# Created: Tue Jul 30 13:22:37 2013 
...
from ui.ui_hklscan import Ui_HKLScan
...
class HKLScan(TaurusWidget):
    ...
    def __init__(self, parent=None, designMode=False):
        ...
        self.connect(self._ui.hklDisplayAnglesButton, Qt.SIGNAL("clicked()"), self.display_angles)
    ...
    def setModel(self,model):
        ...
        for i in range(0,self.nb_motors):
            ...
            angles_labels[i].setGeometry(QtCore.QRect(50 + gap_x * i, 290, 51, 17))
            alname = "angleslabel" + str(i)
            angles_labels[i].setObjectName(alname)
            angles_labels[i].setText(QtGui.QApplication.translate("HKLScan", angles_names[i], None, QtGui.QApplication.UnicodeUTF8))
            angles_taurus_label.append(TaurusLabel(self))
            angles_taurus_label[i].setGeometry(QtCore.QRect(50 + gap_x * i, 320, 81, 19))
        ...
        if self.door_device != None:
            ...
            if dim > 0:
                ...
                for i in range(len(index_to_scan)):
                    macro_command.append(str(self.pseudo_motor_names[index_to_scan[i]]))
    ...
    def display_angles(self):
        ...
        for i in range(0, self.nb_motors):
            ...
            dsa_label[i].setText(QtGui.QApplication.translate("Form", angles_names[i], None, QtGui.QApplication.UnicodeUTF8))
        ...
        for i in range(0, nb_points + 1):
            ...
            if not no_trajectories:
                ...
                for iangle in range(0, self.nb_motors):
                    ...
                    taurusValueAngle[i][iangle].setGeometry(QtCore.QRect(xangle[iangle], yhkl + 30*(i+1), 80, 27))
                    ...
                    taurusValueAngle[i][iangle].setValue("%10.4f" % angles_list[iangle])
            else:
                taurusValueAngle.append(TaurusValueLineEdit(w))
                taurusValueAngle[i].setGeometry(QtCore.QRect(xangle[0], yhkl + 30*(i+1), self.nb_motors * 120, 27))
                taurusValueAngle[i].setReadOnly(True)
                tva_name = "taurusValueAngle" + str(i)
                taurusValueAngle[i].setObjectName(tva_name)
                taurusValueAngle[i].setValue("...                             No angle solution for hkl values                             ...")
        if nb_points == -1:
            nb_points = 0
            taurusValueAngle.append(TaurusValueLineEdit(w))
            taurusValueAngle[0].setGeometry(QtCore.QRect(xangle[0], yhkl + 30, self.nb_motors * 120, 27))
            taurusValueAngle[0].setReadOnly(True)
            tva_name = "taurusValueAngle"
            taurusValueAngle[0].setObjectName(tva_name)
            taurusValueAngle[0].setValue("...          No scan parameters filled. Fill them in the main window         ...")
