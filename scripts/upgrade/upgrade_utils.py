...
class Upgrade:
    ...
    def upgrade(self, db, serv, old, new):
        raise RuntimeError(
            "Invalid Upgrade class. Must define a upgrade method")

    @classmethod
    def fromTo(cls):
        raise RuntimeError(
            "Invalid Upgrade class. Must define a fromTo method")

    @classmethod
    def supports(cls, old, new):
        raise RuntimeError(
            "Invalid Upgrade class. Must define a supports method")

    @classmethod
    def supports_old(cls, old):
        raise RuntimeError(
            "Invalid Upgrade class. Must define a supports_old method")

    @classmethod
    def supports_new(cls, new):
        raise RuntimeError(
            "Invalid Upgrade class. Must define a supports_new method")
...
def get_supported_upgrades():
    ...
    for mod_name in os.listdir(curr_dir):
        ...
        if mod_name == __name__:
            continue
        if mod_name.startswith("upgrade_"):
            continue
